...
async function baixarRelatorioConsumoJson() {
    const dataInicio = document.getElementById('data-inicio')?.value;
    const dataFim = document.getElementById('data-fim')?.value;
    ...
    try {
        const { blob, nome } = await api.baixar_relatorio_consumo_json(dataInicio, dataFim);
        ...
        Util.mostrarToast(`JSON salvo: ${nome} (também em data/saida/consumos/)`, 'success');
    } catch (e) {
        Util.mostrarToast(e.message || 'Erro ao baixar JSON de consumo', 'error');
    }
}
...
window.baixarRelatorioConsumoJson = baixarRelatorioConsumoJson;
