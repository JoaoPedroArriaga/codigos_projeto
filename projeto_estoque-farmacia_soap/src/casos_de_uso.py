...
class CasoDeUsoConsulta:
    ...
    def processar_consulta(self, codigo_medicamento: int, quantidade: int, cpf_paciente: str) -> Dict[str, Any]:
        """Processa uma consulta de disponibilidade usando FEFO (baseado em resposta.xsd)"""
                ...
                'respostas': [
                    {
                        'codigo_medicamento': codigo_medicamento,
                        'disponivel': 0,
                        'observacao': f'Medicamento {codigo_medicamento} não encontrado'
                    }
                ]
            }
        ...
        if lote:
            observacao = f'Disponível em {lote["numero_lote"]} - Validade: {lote["data_validade"].strftime("%d/%m/%Y")}'
            return {
                'respostas': [
                    {
                        'codigo_medicamento': codigo_medicamento,
            ...
            'respostas': [
                {
                    'codigo_medicamento': codigo_medicamento,
                    'disponivel': 0,
                    'observacao': f'Quantidade insuficiente. Solicitado: {quantidade} un.'
                }
            ]
        }
...
class CasoDeUsoReserva:
    ...
    def criar_reserva_com_fefo(self, codigo_medicamento: int, quantidade: int, cpf_paciente: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not id_reserva:
                raise Exception('Falha ao inserir reserva no banco de dados')
            ...
            logger.error(f"Erro ao criar reserva FEFO para medicamento {codigo_medicamento}: {str(e)}", exc_info=True)
            # Re-lançar a exceção para que a transação seja adequadamente tratada
            raise
    ...
    def cancelar_reserva(self, id_reserva: int) -> Dict[str, Any]:
        ...
        sucesso = self.repo_reserva.atualizar(id_reserva, {'status': 'CANCELADO'})
...
class CasoDeUsoBaixa:
    ...
    def dar_baixa(self, codigo_medicamento: int, quantidade: int, 
        ...
        try:
            ...
            logger.error(f"Erro ao dar baixa do medicamento {codigo_medicamento}: {str(e)}", exc_info=True)
            raise