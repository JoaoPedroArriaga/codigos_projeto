...
from src.processors.status_processor import StatusFinanceiroProcessor
from src.processors.finalizacao_processor import FinalizacaoProcessor
from src.utils.xml_utils import listar_arquivos_entrada

def processar_tudo():
    """Processa todos os arquivos pendentes"""
    logger.info("🔍 Verificando arquivos...")
    
    # Processar arquivos do nosso domínio
    ...
    # Processar arquivos externos (apenas validação)
    arquivos = listar_arquivos_entrada('status_financeiro')
    for arquivo in arquivos:
        StatusFinanceiroProcessor.processar(arquivo)
    
    arquivos = listar_arquivos_entrada('finalizacao')
    for arquivo in arquivos:
        FinalizacaoProcessor.processar(arquivo)
